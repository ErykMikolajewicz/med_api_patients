...
from src.texts.patients import EmailVerificationText
...
async def check_email(session, verification_parameter: str) -> UUID | None:
...
async def verify_email(session, patient_id: UUID) -> str:
...
async def send_verification_email(patient_id: UUID, email: str, pool):
    ...
    async with pool.acquire() as session:
        email_verification_data_access = EmailVerification(session)
        await email_verification_data_access.add(verification_parameter, patient_id)

    med_app_email = med_app_email_secrets['email']
    med_app_password = med_app_email_secrets['password']

    subject = EmailVerificationText.email_subject
